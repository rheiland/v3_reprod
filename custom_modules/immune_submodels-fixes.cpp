...
void replace_out_of_bounds_cell( Cell* pC , double tolerance )
{
	...
	// #pragma omp critical  // (kill_cell_on_edge)
	{
		...
		std::cout << __FUNCTION__ << " --- position (for new one) " <<  position << std::endl; 
...
void keep_immune_cells_off_edge( void )
{
	...
	//rwh static bool setup_done = false; 
	bool setup_done = false;   //rwh
...
void keep_immune_cells_in_bounds( double dt )
{
	...
	//rwh static double next_time = 0.0; 
	double next_time = 0.0; 

	// static double t_bounds = 0.0;   // rwh: remove 'static's
	// static double t_last_bounds = 0.0; 
	// static double t_next_bounds = 0.0; 
	double t_bounds = 0.0; 
	double t_last_bounds = 0.0; 
	double t_next_bounds = 0.0; 
